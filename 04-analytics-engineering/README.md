...
- run dbt init in this course folder
- follow local_setup.md from https://github.com/DataTalksClub/data-engineering-zoomcamp/blob/main/04-analytics-engineering/setup/local_setup.md to install duckdb and dbt if not yet instsalled
- run `db init`, setup project name (e.g taxi_rides_ny)
- all command and instruction after this will run inside that project folder
- copy script to load the duckdb data from local_setup.md
- update the script to read file that i already downloaded from script ./load_taxi_data.py
- run the script `python duck_db_ingest.py`
- run `db init`, setup project name (e.g taxi_rides_ny)
- update the dbt profiles, i prefer to put it on this project folder

- run `dbt build` to prepare all data
- run `dbt seed --target prod` and `dbt run --target prod`

### Answer

- Q1. int_trips_unioned only, it will error if we didn't build the staging model. to run upstream, downstream or both, we can user `+`

- Q2. will failed

- Q3. `duckdb taxi_rides_ny.duckdb "SELECT COUNT(*) FROM prod.fct_monthly_zone_revenue;"` got 15579

- Q4. `duckdb taxi_rides_ny.duckdb "SELECT pickup_zone, SUM(revenue_monthly_total_amount) as total_revenue FROM prod.fct_monthly_zone_revenue WHERE service_type = 'Green' AND EXTRACT(YEAR FROM revenue_month) = 2020 GROUP BY pickup_zone ORDER BY total_revenue DESC LIMIT 5;"`

- Q5. `duckdb taxi_rides_ny.duckdb "SELECT SUM(total_monthly_trips) as total_trips FROM prod.fct_monthly_zone_revenue WHERE service_type = 'Green' AND revenue_month = '2019-10-01';"`

- Q6. 43,244,693
  - load the data using `load_fhv_data.py`
  - check the schema `duckdb taxi_rides_ny.duckdb "DESCRIBE prod.fhv_tripdata;"`
  - create schema and model
  - run the model `dbt run --select stg_fhv_tripdata hw_q6_fhv_record_count --target prod`
  - `duckdb taxi_rides_ny.duckdb "SELECT * FROM prod.hw_q6_fhv_record_count;"`
